...
pub async fn create_pool(database_url: &str) -> Result<DbPool, sqlx::Error> {
    ...
    if let Some(host_start) = database_url.find('@')
        && let Some(host_end) = database_url[host_start..].find(':')
        && let Some(port_start) = database_url[host_start + host_end..].find(':')
        && let Some(port_end) = database_url[host_start + host_end + port_start + 1..].find('/') {
...
fn mask_password_in_url(url: &str) -> String {
    ...
    if let Some(at_pos) = url.find('@')
        && let Some(slash_pos) = url[..at_pos].rfind(':')
        && let Some(user_pos) = url[..slash_pos].rfind("://") {
